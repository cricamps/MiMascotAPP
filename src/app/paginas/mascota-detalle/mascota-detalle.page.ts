...
export class MascotaDetallePage implements OnInit {
  ...
  constructor(private mascotaService: MascotasService,
    ...
    async loadMascota(){
      ...
      this.mascotaService.getMascota(this.mascotaId).subscribe((res: any) => {
        loading.dismiss();
    ...
    enumSelector(definition: typeof Especie) {
    ...
    async subirImagen(){
        ...
        if(this.mascota.imagen != '' && this.imagenActualizada){
          ...
          this.mascotaService.removeImagenMascota(this.mascota.imagen).then(async (r: any) => {
            loading.dismiss();
  ...
  async subirImagenGuardarMascota(){
    ...
    this.mascotaService.uploadImagenMascota(this.imagen).then((savedPicture: { ref: { getDownloadURL: () => Promise<any>; }; }) => {
      savedPicture.ref.getDownloadURL().then(async (url: string) =>  {
