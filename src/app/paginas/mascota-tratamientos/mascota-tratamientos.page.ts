...
export class MascotaTratamientosPage implements OnInit {
  ...
  mascota!: Mascota;
  tipoTratamiento: TipoTratamiento = "Vacunas";
  ...
  async loadTratamientos(){
    ...
    this.mascotaService.getTratamientos(this.mascotaId).subscribe((res: any[]) => {
      this.tratamientos = res.filter((t: { tipo: TipoTratamiento; }) => t.tipo == this.tipoTratamiento ).sort((a: { proximo_tratamiento: string | number | Date; fecha: string | number | Date; }, b: { proximo_tratamiento: string | number | Date; fecha: string | number | Date; }) => {
